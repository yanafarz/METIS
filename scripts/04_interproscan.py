...
def ask_output(input_file):
    ...
    output_file = os.path.join(folder, filename + ".tsv")
...
def main():
    ...
    section("STEP 1: INPUT PROTEIN FASTA")

    input_file = expand_path(
        input("Filtered fasta path: ")
    )

    if not check_file_exists(input_file):
        return

    sequence_count = count_fasta_sequences(input_file)
    ...
    print("\n✔ FASTA detected.")
    print(f"✔ Sequences found: {sequence_count:,}")

    section("STEP 2: INTERPROSCAN")
    ...
    output_file = ask_output(input_file)

    section("STEP 4: RUN INTERPROSCAN")
    ...
    section("INTERPROSCAN SUMMARY")

    print("INPUT :", input_file)
    print("SEQUENCES:", f"{sequence_count:,}")
    print("INTERPROSCAN:", interproscan_dir)
    print("OUTPUT:", output_file)

    if success:
        add_interproscan_header(output_file)
